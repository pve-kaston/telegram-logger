...
_MD_SAFE_RE = re.compile(r"^[A-Za-z0-9 .-]+$")
...
async def _create_mention(client, entity_id: int, chat_msg_id: int | None = None) -> str:
    ...
    try:
        ...
        if first:
            ...
            if not _is_md_safe(full_name):
                full_name = f"User {entity_id}"
